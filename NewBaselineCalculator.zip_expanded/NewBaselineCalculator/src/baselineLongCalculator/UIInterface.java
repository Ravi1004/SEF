...
//	private boolean operand2Defined = false;
	...
	public UIInterface (Pane root, double ww) {
	...
	private void initializeTheUserInterface(Pane theRoot) {
		...
		button_Add.setOnAction((event) -> { if(!binaryOperandIssues()) {label_errOperand1.setText("");
		label_errOperand2.setText("");
		label_Result.setText("Sum");
		text_Result.setText(BL.addOperands());								// result, place in it the output, and
		...
		button_Sub.setOnAction((event) -> {if(!binaryOperandIssues()) {label_errOperand1.setText("");
		label_errOperand2.setText("");
		label_Result.setText("sub");
		text_Result.setText(BL.subOperands());								// result, place in it the output, and
		}});
		...
		button_Mpy.setOnAction((event) -> {if(!binaryOperandIssues()) {label_errOperand1.setText("");
		label_errOperand2.setText("");
		label_Result.setText("mpy");
		text_Result.setText(BL.mpyOperands());								// result, place in it the output, and
		}});
		...
		button_Div.setOnAction((event) -> { if (!binaryOperandIssues()) { label_errOperand2.setText(" ");label_errOperand1.setText(" ");
		if (BL.divOperands() == "Error") {
			label_Result.setText("Division");
			label_errOperand1.setText("");
			label_errOperand2.setText("Error: division by 0 Not Possible");
		} else {
			label_Result.setText("Division");
			text_Result.setText(BL.divOperands());
			}
		}});
	...
	private void convertOperand1() {
		BL.operand1Defined = false;									// Assume the first operand is
		...
		BL.operand1 = new CalculatorValue(str);						// There is a string so try to
		String errorString = BL.operand1.getErrorMessage();			// make and operand out of it
		if (errorString.length() > 0) {								// If there is an error the
			label_errOperand1.setText(errorString);					// returned error string is used
			BL.operandError = true;
			return;
		}
		
		text_Result.setText("");									// Clear the result field
		label_errOperand1.setText("");								// Reset the error message
		BL.operand1Defined = true;										// The first operand is defined
		BL.operandError = false;										// Indicate there is no error
		}	
	...
	private void convertOperand2() {
		BL.operand2Defined = false;									// Assume the first operand is
		label_Result.setText("Result");								// not valid, reset the label,
		text_Result.setText("");									// and clear the result
		// Fetch the string the user entered into the operand's text field
		...
		BL.operand2 = new CalculatorValue(str);						// There is a string so try to
		String errorString = BL.operand2.getErrorMessage();			// make and operand out of it
		if (errorString.length() > 0) {								// If there is an error the
		...
		text_Result.setText("");									// Clear the result field
		label_errOperand2.setText("");								// Reset the error message
		BL.operand2Defined = true;										// The first operand is defined
		BL.operandError = false;										// Indicate there is no error
		}	
	...
	private boolean binaryOperandIssues() {
		if (!BL.operand1Defined) 										// Check operand 1 and set a
			label_errOperand1.setText("Missing a valid value");		// missing value error
		if (!BL.operand2Defined) 										// Check operand 2 and set a
			label_errOperand2.setText("Missing a valid value");		// missing value error
		if (BL.operandError)											// See if invalid input errors
			return true;											// return true if so
		if (!BL.operand1Defined || !BL.operand2Defined)					// See if any missing input values
