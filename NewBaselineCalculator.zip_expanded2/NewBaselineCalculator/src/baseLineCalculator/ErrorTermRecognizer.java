	...
	public static String checkErrorTerm(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// If the current character is range of 1 to 9, it transitions to state 1
				else if (currentChar >= '1' && currentChar <= '9') {
					nextState = 1;
					break;					
				}
				// If the current chracter is a decimal point then it transitions to state 3
				else if (currentChar == '.') {
					nextState = 3;
					break;					
				}
			...
			case 1: 
				...
				// In state 1, if the character is 0, it is accepted and we move to the next state
				if (currentChar == '0') {
					nextState = 1;
					break;
				}
			...
			case 4: 
				// State 4 has only one valid transitions.  Each is addressed by an if statement.
				// This is a final state
				finalState = true;
				// If the current character is an 'E' or 'e", it transitions to state 5
				if (currentChar == 'E' || currentChar == 'e') {
			...
			case 6: 
				// State 6 has only one valid transitions.  Each is addressed by an if statement.
				// This is not a final state
				finalState = false;
				// If the current character is in the range from 0 to 9, it transitions to state 7
				if (currentChar >= '0' && currentChar <= '9') {
			...
			case 7: 
				// State 7 has only one valid transitions.  Each is addressed by an if statement.
				// This a final state
				finalState = true;
				// If the current character is in the range from 0 to 9, it transitions to state 7
				if (currentChar >= '0' && currentChar <= '9') {
			...
			case 8:
				// State 8 has only one valid transitions.  Each is addressed by an if statement.
		...
		case 5:
			...
			return errorTermErrorMessage + displayInput(input, currentCharNdx);
		case 8:
			errorTermErrorMessage = "This character should be a dot .\n";
			return errorTermErrorMessage + displayInput(input, currentCharNdx);
		default:
			return "";
		}
